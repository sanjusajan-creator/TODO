    ...
    try {
      if (pathname === '/api/auth/register' && method === 'POST') {
        ...
        if (existing.rows.length > 0) {
          return new Response(JSON.stringify({ message: 'Email already exists' }), { status: 400, headers: { 'Content-Type': 'application/json' }}
      ...
      if (pathname.startsWith('/api/tasks/') && method === 'PUT') {
        ...
        if (result.rows.length === 0) {
          return new Response(JSON.stringify({ message: 'Task not found' }), { status: 404, headers: { 'Content-Type': 'application/json' }})
        }
